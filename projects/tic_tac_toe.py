...
class TicTacToe:
    ...
    def __init__(self, master):
        ...
        # Define buttons
        self.btn_a1 = tk.Button(
            master, font=self.button_font, height=self.button_height, width=self.button_width
        )
        self.btn_a2 = tk.Button(
            master, font=self.button_font, height=self.button_height, width=self.button_width
        )
        self.btn_a3 = tk.Button(
            master, font=self.button_font, height=self.button_height, width=self.button_width
        )
        ...
        self.btn_b3 = tk.Button(
            master, font=self.button_font, height=self.button_height, width=self.button_width
        )
        self.btn_c1 = tk.Button(
            master, font=self.button_font, height=self.button_height, width=self.button_width
        )
        self.btn_c2 = tk.Button(
            master, font=self.button_font, height=self.button_height, width=self.button_width
        )
        self.btn_c3 = tk.Button(
            master, font=self.button_font, height=self.button_height, width=self.button_width
        )
        ...
        self.bottom_label.grid(
            row=4, column=1, columnspan=2, rowspan=2, sticky=(tk.N + tk.S + tk.W + tk.E)
        )
    ...
    def take_cpu_turn(self) -> None:
        ...
        # A block
            ...
            ("A2" in self.user_chosen_cells and "A3" in self.user_chosen_cells)
            ...
            ("A1" in self.user_chosen_cells and "A3" in self.user_chosen_cells)
            or ("B2" in self.user_chosen_cells and "C2" in self.user_chosen_cells)
        ) and "A2" in self.available_cells:
            self.cpu_choice = "A2"
        elif (
            ("A1" in self.user_chosen_cells and "A2" in self.user_chosen_cells)
        ...
        # B block
            ...
            ("B2" in self.user_chosen_cells and "B3" in self.user_chosen_cells)
            or ("A1" in self.user_chosen_cells and "C1" in self.user_chosen_cells)
        ) and "B1" in self.available_cells:
            self.cpu_choice = "B1"
        elif (
            ("B1" in self.user_chosen_cells and "B3" in self.user_chosen_cells)
            ...
            ("B1" in self.user_chosen_cells and "B2" in self.user_chosen_cells)
            or ("A3" in self.user_chosen_cells and "C3" in self.user_chosen_cells)
        ) and "B3" in self.available_cells:
            self.cpu_choice = "B3"
        # C block
        elif (
            ("C2" in self.user_chosen_cells and "C3" in self.user_chosen_cells)
            ...
            ("C1" in self.user_chosen_cells and "C3" in self.user_chosen_cells)
            or ("A2" in self.user_chosen_cells and "B2" in self.user_chosen_cells)
        ) and "C2" in self.available_cells:
            self.cpu_choice = "C2"
        elif (
            ("C1" in self.cpu_chosen_cells and "C2" in self.user_chosen_cells)
            or ("B3" in self.user_chosen_cells and "C3" in self.user_chosen_cells)
            or ("C1" in self.user_chosen_cells and "B2" in self.user_chosen_cells)
        ...
        # A block
            ...
            ("A2" in self.cpu_chosen_cells and "A3" in self.cpu_chosen_cells)
            ...
            ("A1" in self.cpu_chosen_cells and "A3" in self.cpu_chosen_cells)
            or ("B2" in self.cpu_chosen_cells and "C2" in self.cpu_chosen_cells)
        ) and "A2" in self.available_cells:
            self.cpu_choice = "A2"
        elif (
            ("A1" in self.cpu_chosen_cells and "A2" in self.cpu_chosen_cells)
        ...
        # B block
            ...
            ("B2" in self.cpu_chosen_cells and "B3" in self.cpu_chosen_cells)
            or ("A1" in self.cpu_chosen_cells and "C1" in self.cpu_chosen_cells)
        ) and "B1" in self.available_cells:
            self.cpu_choice = "B1"
        elif (
            ("B1" in self.cpu_chosen_cells and "B3" in self.cpu_chosen_cells)
            ...
            ("B1" in self.cpu_chosen_cells and "B2" in self.cpu_chosen_cells)
            or ("A3" in self.cpu_chosen_cells and "C3" in self.cpu_chosen_cells)
        ) and "B3" in self.available_cells:
            self.cpu_choice = "B3"
        # C block
        elif (
            ("C2" in self.cpu_chosen_cells and "C3" in self.cpu_chosen_cells)
            ...
            ("C1" in self.cpu_chosen_cells and "C3" in self.cpu_chosen_cells)
            or ("A2" in self.cpu_chosen_cells and "B2" in self.cpu_chosen_cells)
        ) and "C2" in self.available_cells:
            self.cpu_choice = "C2"
        elif (
            ("C1" in self.cpu_chosen_cells and "C2" in self.cpu_chosen_cells)
            or ("B3" in self.cpu_chosen_cells and "C3" in self.cpu_chosen_cells)
        ...
        # CPU start
            ...
            self.cpu_choice = random.choices(
                ...
                weights=[0.2, 0.2, 0.2, 0.2, 0.1, 0.025, 0.025, 0.025, 0.025],
                k=1,
            )[0]
        ...
        # Check for tie
        if not self.available_cells and self.bottom_label["text"] not in [
            self.cpu_win_msg,
            self.user_win_msg,
        ]:
            self.bottom_label["text"] = self.tie_msg
